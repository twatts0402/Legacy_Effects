...
# This script asks whether plant communities respond to snow conditions from
# three years earlier, which is a simple three-year ecological memory / legacy effect.
#
# Snow predictor in this script:
#   three_year_lag_snow_depth = mean annual snow depth from three calendar years
#   earlier for the same Saddle Grid plot/stake.
...
snow_annual <- read_csv(snow_path, na = c("", "NA", "NaN"), show_col_types = FALSE) |>
  ...
  arrange(plot, year) |>
  group_by(plot) |>
  mutate(three_year_lag_snow_depth = lag(mean_annual_snow_depth, n = 3)) |>
  ungroup()
...
richness_snow <- vegetation_plot_year |>
  left_join(
    snow_annual |> select(plot, year, three_year_lag_snow_depth),
    by = c("plot", "year")
  ) |>
  filter(!is.na(three_year_lag_snow_depth), !is.na(species_richness))

richness_model <- lm(species_richness ~ three_year_lag_snow_depth, data = richness_snow)
richness_model_summary <- summary(richness_model)

richness_r_squared <- richness_model_summary$r.squared
richness_p_value <- richness_model_summary$coefficients["three_year_lag_snow_depth", "Pr(>|t|)"]

richness_plot <- ggplot(
  richness_snow,
  aes(x = three_year_lag_snow_depth, y = species_richness)
) +
  geom_point(alpha = 0.55) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  labs(
    x = "Snow depth three years earlier",
    y = "Species richness",
    title = "Species richness vs. snow depth three years earlier"
  ) +
  annotate(
    "text",
    x = Inf,
...
ggsave(
  filename = file.path(output_folder, "03_three_year_lag_snow_depth_richness.png"),
  plot = richness_plot,
  width = 7,
  height = 5,
  dpi = 300
...
species_snow <- vegetation_species |>
  ...
  filter(!is.na(three_year_lag_snow_depth), !is.na(relative_cover))
...
species_models <- species_snow |>
  ...
  mutate(
    ...
    model = map2(
      ...
      cover_values,
      ~ if (nrow(.x) >= 10 && .y > 1) {
        lm(relative_cover ~ three_year_lag_snow_depth, data = .x)
      } else {
        NULL
      }
    ),
    ...
    p_value = map_dbl(
      ...
      ~ if (is.null(.x)) {
        ...
        .x$coefficients["three_year_lag_snow_depth", "Pr(>|t|)"]
      }
    ),
    slope = map_dbl(
      model,
      ~ if (is.null(.x)) NA_real_ else coef(.x)[["three_year_lag_snow_depth"]]
    )
  ) |>
...
species_models_to_save <- species_models |>
  mutate(
    slope = format(round(slope, 6), scientific = FALSE),
    r_squared = format(round(r_squared, 4), scientific = FALSE),
    p_value = format(round(p_value, 6), scientific = FALSE)
  )

write_csv(
  species_models_to_save,
  file.path(output_folder, "03_three_year_lag_snow_depth_species_models.csv")
)
...
species_plot <- ggplot(
  species_plot_data,
  aes(x = three_year_lag_snow_depth, y = relative_cover)
) +
  ...
  labs(
    x = "Snow depth three years earlier",
    y = "Relative cover",
    title = "Relative cover of common species vs. snow depth three years earlier"
  ) +
  theme_bw()

ggsave(
  filename = file.path(output_folder, "03_three_year_lag_snow_depth_species_relative_cover.png"),
  plot = species_plot,
  width = 12,
  height = 9,
  dpi = 300
