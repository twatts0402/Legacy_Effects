...
# This script asks whether plant communities respond to accumulated snow history
# across a five-year window.
#
# Snow predictor in this script:
#   rolling_5yr_snow_depth = mean annual snow depth averaged across the current
#   year and the four previous years for the same Saddle Grid plot/stake.
#
# To make this a strictly lagged legacy metric that excludes the current year,
# average lag_1 through lag_5 instead of mean_annual_snow_depth through lag_4.
...
snow_annual <- read_csv(snow_path, na = c("", "NA", "NaN"), show_col_types = FALSE) |>
  ...
  group_by(plot) |>
  mutate(
    lag_1 = lag(mean_annual_snow_depth, n = 1),
    lag_2 = lag(mean_annual_snow_depth, n = 2),
    lag_3 = lag(mean_annual_snow_depth, n = 3),
    lag_4 = lag(mean_annual_snow_depth, n = 4),
    rolling_5yr_snow_depth = rowMeans(
      pick(mean_annual_snow_depth, lag_1, lag_2, lag_3, lag_4),
      na.rm = FALSE
    )
  ) |>
  ungroup()
...
richness_snow <- vegetation_plot_year |>
  left_join(
    snow_annual |> select(plot, year, rolling_5yr_snow_depth),
    by = c("plot", "year")
  ) |>
  filter(!is.na(rolling_5yr_snow_depth), !is.na(species_richness))

richness_model <- lm(species_richness ~ rolling_5yr_snow_depth, data = richness_snow)
richness_model_summary <- summary(richness_model)

richness_r_squared <- richness_model_summary$r.squared
richness_p_value <- richness_model_summary$coefficients["rolling_5yr_snow_depth", "Pr(>|t|)"]

richness_plot <- ggplot(
  richness_snow,
  aes(x = rolling_5yr_snow_depth, y = species_richness)
) +
  geom_point(alpha = 0.55) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  labs(
    x = "5-year rolling mean snow depth",
    y = "Species richness",
    title = "Species richness vs. 5-year rolling mean snow depth"
  ) +
  annotate(
    "text",
    x = Inf,
...
ggsave(
  filename = file.path(output_folder, "05_five_year_rolling_snow_depth_richness.png"),
  plot = richness_plot,
  width = 7,
  height = 5,
  dpi = 300
...
species_snow <- vegetation_species |>
  ...
  filter(!is.na(rolling_5yr_snow_depth), !is.na(relative_cover))
...
species_models <- species_snow |>
  ...
  mutate(
    ...
    model = map2(
      ...
      cover_values,
      ~ if (nrow(.x) >= 10 && .y > 1) {
        lm(relative_cover ~ rolling_5yr_snow_depth, data = .x)
      } else {
        NULL
      }
    ),
    ...
    p_value = map_dbl(
      ...
      ~ if (is.null(.x)) {
        ...
        .x$coefficients["rolling_5yr_snow_depth", "Pr(>|t|)"]
      }
    ),
    slope = map_dbl(
      model,
      ~ if (is.null(.x)) NA_real_ else coef(.x)[["rolling_5yr_snow_depth"]]
    )
  ) |>
...
write_csv(
  species_models,
  file.path(output_folder, "05_five_year_rolling_snow_depth_species_models.csv")
)
...
species_plot <- ggplot(
  species_plot_data,
  aes(x = rolling_5yr_snow_depth, y = relative_cover)
) +
  ...
  labs(
    x = "5-year rolling mean snow depth",
    y = "Relative cover",
    title = "Relative cover of common species vs. 5-year rolling mean snow depth"
  ) +
  theme_bw()

ggsave(
  filename = file.path(output_folder, "05_five_year_rolling_snow_depth_species_relative_cover.png"),
  plot = species_plot,
  width = 12,
  height = 9,
  dpi = 300
