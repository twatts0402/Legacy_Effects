...
# five years earlier, which is a simple five-year ecological memory / legacy effect.
#
# Snow predictor in this script:
#   five_year_lag_snow_depth = mean annual snow depth from five calendar years
...
snow_annual <- read_csv(snow_path, na = c("", "NA", "NaN"), show_col_types = FALSE) |>
  ...
  arrange(plot, year) |>
  group_by(plot) |>
  mutate(five_year_lag_snow_depth = lag(mean_annual_snow_depth, n = 5)) |>
  ungroup()
...
richness_snow <- vegetation_plot_year |>
  left_join(
    snow_annual |> select(plot, year, five_year_lag_snow_depth),
    by = c("plot", "year")
  ) |>
  filter(!is.na(five_year_lag_snow_depth), !is.na(species_richness))

richness_model <- lm(species_richness ~ five_year_lag_snow_depth, data = richness_snow)
richness_model_summary <- summary(richness_model)

richness_r_squared <- richness_model_summary$r.squared
richness_p_value <- richness_model_summary$coefficients["five_year_lag_snow_depth", "Pr(>|t|)"]

richness_plot <- ggplot(
  richness_snow,
  aes(x = five_year_lag_snow_depth, y = species_richness)
) +
  geom_point(alpha = 0.55) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  labs(
    x = "Snow depth five years earlier",
    y = "Species richness",
    title = "Species richness vs. snow depth five years earlier"
  ) +
  annotate(
    "text",
    x = Inf,
...
ggsave(
  filename = file.path(output_folder, "04_five_year_lag_snow_depth_richness.png"),
  plot = richness_plot,
  width = 7,
  height = 5,
  dpi = 300
...
species_snow <- vegetation_species |>
  ...
  filter(!is.na(five_year_lag_snow_depth), !is.na(relative_cover))
...
species_models <- species_snow |>
  ...
  mutate(
    ...
    model = map2(
      ...
      cover_values,
      ~ if (nrow(.x) >= 10 && .y > 1) {
        lm(relative_cover ~ five_year_lag_snow_depth, data = .x)
      } else {
        NULL
      }
    ),
    ...
    p_value = map_dbl(
      ...
      ~ if (is.null(.x)) {
        ...
        .x$coefficients["five_year_lag_snow_depth", "Pr(>|t|)"]
      }
    ),
    slope = map_dbl(
      model,
      ~ if (is.null(.x)) NA_real_ else coef(.x)[["five_year_lag_snow_depth"]]
    )
  ) |>
...
write_csv(
  ...
  file.path(output_folder, "04_five_year_lag_snow_depth_species_models.csv")
)
...
species_plot <- ggplot(
  species_plot_data,
  aes(x = five_year_lag_snow_depth, y = relative_cover)
) +
  ...
  labs(
    x = "Snow depth five years earlier",
    y = "Relative cover",
    title = "Relative cover of common species vs. snow depth five years earlier"
  ) +
  theme_bw()

ggsave(
  filename = file.path(output_folder, "04_five_year_lag_snow_depth_species_relative_cover.png"),
  plot = species_plot,
  width = 12,
  height = 9,
  dpi = 300
