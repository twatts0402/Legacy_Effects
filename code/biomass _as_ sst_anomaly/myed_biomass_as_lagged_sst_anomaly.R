...
#the purpose of this program is to find a correlation of myed biomass to previous months sst anomaly

biomass <- read_csv("/Users/tobiahwatts/Downloads/biomass_data_stacked.csv")

# Get monthly myed biomass
myed_biomass <- biomass |> 
  mutate(month_date = as.Date(paste(year, month, "01", sep = "-"))) |> 
  filter(biomass.species == "MYED") |> 
  group_by(month_date) |> 
  summarise(total_myed_biomass = sum(biomass, na.rm = TRUE))
...
# Join MYED biomass with previous month SST gets rid of rows w/ na values
myed_lagged <- myed_biomass |> 
  left_join(
...
model <- lm(total_myed_biomass ~ previous_month_sst, data = myed_lagged)
...
ggplot(myed_lagged,
       aes(x = previous_month_sst,
           y = total_myed_biomass)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "MYED Biomass vs Previous Month SST Anomaly",
    x = "Previous Month Sea Surface Temperature Anomaly (°C)",
    y = "Total MYED Biomass") +
