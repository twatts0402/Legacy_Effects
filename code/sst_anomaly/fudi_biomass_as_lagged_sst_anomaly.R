...
#the purpose of this program is to find a correlation of FUDI biomass to previous months sst anomaly

biomass <- read_csv("/Users/tobiahwatts/Downloads/biomass_data_stacked.csv")

# Get monthly FUDI biomass
fudi_biomass <- biomass |> 
  mutate(month_date = as.Date(paste(year, month, "01", sep = "-"))) |> 
  filter(biomass.species == "FUDI") |> 
  group_by(month_date) |> 
  summarise(total_fudi_biomass = sum(biomass, na.rm = TRUE))
...
# Join FUDI biomass with previous month SST gets rid of rows w/ na values
fudi_lagged <- fudi_biomass |> 
  left_join(
...
model <- lm(total_fudi_biomass ~ previous_month_sst, data = fudi_lagged)
...
ggplot(fudi_lagged,
       aes(x = previous_month_sst,
           y = total_fudi_biomass)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "FUDI Biomass vs Previous Month SST Anomaly",
    x = "Previous Month Sea Surface Temperature Anomaly (°C)",
    y = "Total FUDI Biomass") +
